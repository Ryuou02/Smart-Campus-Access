...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Map<String, bool> _isGeneratingId = {};
  List<String> _selectedCoursesForAssignment = [];
  ...
  Future<void> _fetchCourses() async {
    final courses = await mongo.MongoDBService.getAllCourses();
    setState(() {
      _courses = courses;
    });
  }

  Future<List<String>> _fetchAssignedCourses(String rollNumber) async {
    final studentCourses = await mongo.MongoDBService.getCoursesForStudent(rollNumber);
  ...
  Future<void> _createCourse() async {
    if (_courseFormKey.currentState!.validate()) {
      ...
      final course = {
        ...
        'resources': {
          'syllabusLink': _syllabusLink,
          'scheduleLink': _scheduleLink,
          'materialsLink': _materialsLink,
        },
        'createdAt': DateTime.now().toIso8601String(),
      };

      await mongo.MongoDBService.createCourse(course);
      await _fetchCourses();
  ...
  Future<void> _assignCoursesToStudent(String rollNumber) async {
    try {
      await mongo.MongoDBService.assignCoursesToStudent(rollNumber, _selectedCoursesForAssignment);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Courses assigned successfully")),
      );
      setState(() {
        _isAssigningCourses = false;
        _selectedCoursesForAssignment = [];
      });
    } catch (e) {
      print("Error in _assignCoursesToStudent: $e");
      ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            : SingleChildScrollView(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    _buildElevatedButton(
                                      onPressed: () async {
                                        print("View Students button clicked");
                                        try {
                                          await _fetchStudents();
                                        } catch (e) {
                                          print("Error during fetchStudents: $e");
                                        } finally {
                                          if (mounted) {
                                            setState(() {
                                              _isViewingStudents = true;
                                              print("Set _isViewingStudents to true");
                                            });
                                          }
                                        }
                                      },
      ...
      child: Form(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 15),
            _buildTextField(
              label: 'Syllabus Link (Google Drive)',
              icon: Icons.link,
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Please enter the syllabus link';
                }
              ...
              onSaved: (value) => _syllabusLink = value!,
            ),
            const SizedBox(height: 15),
            _buildTextField(
              label: 'Class Schedule Link (Google Drive)',
              icon: Icons.link,
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Please enter the class schedule link';
                }
                if (!Uri.parse(value).isAbsolute) {
                  return 'Please enter a valid URL';
                }
              ...
              onSaved: (value) => _scheduleLink = value!,
            ),
            const SizedBox(height: 15),
            _buildTextField(
              label: 'Additional Materials Link (Google Drive)',
              icon: Icons.link,
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Please enter the additional materials link';
                }
                if (!Uri.parse(value).isAbsolute) {
                  return 'Please enter a valid URL';
                }
              ...
              onSaved: (value) => _materialsLink = value!,
            ),
            const SizedBox(height: 20),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _buildSectionTitle("Courses List"),
              IconButton(
                icon: const Icon(Icons.arrow_back, color: Colors.blueAccent),
                onPressed: () {
                  setState(() {
                    _isViewingCourses = false;
                  });
                },
              ),
            ],
          ...
          const SizedBox(height: 20),
          _courses.isEmpty
              ? _buildEmptyMessage("No courses found")
              : ListView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ListTile(
                                      ...
                                      subtitle: Text("Name: ${student['name'] ?? 'N/A'}"),
                                      onTap: () {
                                        setState(() {
                      ...
                      : _isAssigningCourses
                          ...
                          : Column(
                              children: [
                                ...
                                _buildElevatedButton(
                                  onPressed: () async {
                                    final assignedCourses = await _fetchAssignedCourses(_selectedStudent!['rollNumber']);
                                    setState(() {
                                      _isAssigningCourses = true;
                                      _selectedCoursesForAssignment = assignedCourses;
                                    });
                                  },
  ...
  Widget _buildCourseAssignmentForm() {
    final rollNumber = _selectedStudent!['rollNumber'];
    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionTitle("Assign Courses to $rollNumber"),
          const SizedBox(height: 20),
          _courses.isEmpty
              ? _buildEmptyMessage("No courses available. Please create a course first.")
              : Column(
                  children: _courses.map((course) {
                    ...
                    return CheckboxListTile(
                      ...
                      value: _selectedCoursesForAssignment.contains(courseCode),
                      onChanged: (bool? value) {
                        setState(() {
                          if (value == true) {
                            _selectedCoursesForAssignment.add(courseCode);
                          } else {
                            _selectedCoursesForAssignment.remove(courseCode);
                          }
                        });
                      },
                    );
          ...
          Row(
            ...
            children: [
              ...
              _buildElevatedButton(
                onPressed: () => _assignCoursesToStudent(rollNumber),
